...
def options():
    ...
    assert args.model in ['pointnet', 'pointnet2'], 'Unsupported model architecture {}'.format(args.model)
...
def train(opts):
    ...
    epochs = opts.epochs
    lr = paddle.optimizer.lr.ReduceOnPlateau(opts.lr, factor=0.5, patience=5, threshold=0.01, verbose=True)
    if opts.optim == 'adam':
